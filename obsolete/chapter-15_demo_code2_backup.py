...
stockDataFinal = stockDataFinal[:,:,:N_INPUT_FEATURES]
print(stockDataFinal.shape)
...
def linear_regression_forecasting(x_train,y_train,x_valid,y_valid):
    ...
    y_pred = y_pred.reshape(VALIDATION_BATCH_SIZE,N_PREDICTIONS,N_OUTPUT_FEATURES)
...
def rnn_iterative_forecasting(x_train,y_train,x_valid,y_valid):
    ...
    full_sequence = np.zeros((VALIDATION_BATCH_SIZE,N_STEPS,N_INPUT_FEATURES))
    full_sequence[:,:N_INPUT_STEPS,:] = x_valid[:,:,:] # fill in the input time steps
    for step in range(N_PREDICTIONS):
        offset = N_INPUT_STEPS + step
        full_sequence[:,offset,N_OUTPUT_FEATURES:] = x_valid[:,-1,N_OUTPUT_FEATURES:] # insert Naive input features (constant from the last input step)
        data_out = model.predict(full_sequence[:,step:offset,:],VALIDATION_BATCH_SIZE)
        full_sequence[:,offset,:N_OUTPUT_FEATURES] = data_out.reshape(VALIDATION_BATCH_SIZE,N_OUTPUT_FEATURES) # output features span from index 0 to N_OUTPUT_STEPS
...
def rnn_vector_forecasting(x_train,y_train,x_valid,y_valid):
    ...
    training_history = model.fit(x_train,y_train,epochs=N_EPOCHS,validation_data=(x_valid,y_valid),verbose=0)
    y_pred = model.predict(x_valid, VALIDATION_BATCH_SIZE)
    y_pred = y_pred.reshape(VALIDATION_BATCH_SIZE,N_PREDICTIONS,N_OUTPUT_FEATURES) # Reshape the output into our previously-used format for evaluation
    return training_history.history, y_pred
...
def rnn_sequence_forecasting(x_train,x_valid,series,indices):
    ...
    y_pred = model.predict(x_valid, VALIDATION_BATCH_SIZE)
    y_pred = y_pred.reshape(VALIDATION_BATCH_SIZE,N_INPUT_STEPS,N_PREDICTIONS,N_OUTPUT_FEATURES)[:,-1] #we only need the prediction from the last input time step
    return training_history.history, y_pred
